...
public class CameraActivityLow extends FragmentActivity {
	
	//是否有前后摄像头
	boolean hasTwoCameras=false;
	//是否处于前置摄像头模式
	boolean isFFC=true;
	MyCameraFragmentLow fragment=null;
	...
	public void changeFFC() {
		if(isFFC) {
			fragment=MyCameraFragmentLow.getInstance(false);
		}
		else {
			fragment=MyCameraFragmentLow.getInstance(true);
		}
